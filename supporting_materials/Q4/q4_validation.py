# 文件名：q4_validation.py
# 用途：该代码负责第四问移动边界模型的收敛性、时间积分、守恒性及结构可靠性检验。
...
CACHE_PATH = HERE / "diagnostics" / "q4_validation_cache.json"
...
def run_full(cache: dict) -> dict:
    ...
    validations["validation_runtime_note"] = "网格与敏感性工况的单次运行时间见验证缓存记录。"
