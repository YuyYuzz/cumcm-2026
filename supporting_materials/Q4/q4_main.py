# 文件名：q4_main.py
# 用途：该代码负责第四问考虑药材径向收缩的移动边界模型求解、烘干时间计算及结果表格输出。
...
FIXED_OUTPUT_RADII_M = np.arange(21, dtype=float) / 1000.0  # 0,0.1,...,2.0 cm
...
def record_path(path: Path) -> str:
    """Record a portable path relative to the project root."""
...
def load_ambient(path: Path = ATTACHMENT_1) -> AmbientData:
    ...
    if abs(values[-1, 1] - POST_TEMPERATURE_C) > 1.0e-12 or abs(values[-1, 2] - POST_MOISTURE) > 1.0e-12:
        raise ValueError("附件1末值与规定环境端点不一致")
...
def _extend_template_style(worksheet, row: int, column: int, max_row: int, max_column: int) -> None:
    source = worksheet.cell(min(row, max_row), min(column, max_column))
...
def build_result_workbook(
    solution: Q4Solution,
    output_path: Path = TABLES_DIR / "result4.xlsx",
) -> dict[str, object]:
    """Copy the official template and fill fixed-position and surface moisture values."""
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    if output_path.resolve() == OFFICIAL_TEMPLATE.resolve():
        raise ValueError("拒绝覆盖官方result4.xlsx模板")
    shutil.copy2(OFFICIAL_TEMPLATE, output_path)
    workbook = load_workbook(output_path)
    if len(workbook.worksheets) != 1:
        raise ValueError("官方result4.xlsx应仅包含一个工作表")
    worksheet = workbook.worksheets[0]
    template_rows = worksheet.max_row
    template_columns = worksheet.max_column
    headers: list[float | str] = [
        *np.round(FIXED_OUTPUT_RADII_M * 100.0, 10).tolist(),
        "药材表面",
    ]
    for column, value in enumerate(headers, start=2):
        _extend_template_style(worksheet, 1, column, template_rows, template_columns)
        worksheet.cell(1, column, value)
    regular = (
        (solution.time_s >= OUTPUT_INTERVAL_S - 1.0e-8)
        & (
            np.abs(
                solution.time_s / OUTPUT_INTERVAL_S
                - np.round(solution.time_s / OUTPUT_INTERVAL_S)
            )
            < 1.0e-10
        )
        & (solution.time_s <= solution.end_time_s + 1.0e-8)
    )
    times = solution.time_s[regular]
    fixed_values = solution.moisture_fixed[regular]
    surface_values = solution.moisture_surface[regular]
    for row, (time_value, fixed, surface) in enumerate(
        zip(times, fixed_values, surface_values), start=2
    ):
        if row > template_rows:
            worksheet.row_dimensions[row].height = worksheet.row_dimensions[template_rows].height
        _extend_template_style(worksheet, row, 1, template_rows, template_columns)
        worksheet.cell(row, 1, int(round(float(time_value))))
        for column, value in enumerate(fixed, start=2):
            _extend_template_style(worksheet, row, column, template_rows, template_columns)
            cell = worksheet.cell(row, column)
            cell.value = None if not np.isfinite(value) else round(float(value), 4)
            cell.number_format = "0.0000"
        surface_column = len(FIXED_OUTPUT_RADII_M) + 2
        _extend_template_style(
            worksheet, row, surface_column, template_rows, template_columns
        )
        surface_cell = worksheet.cell(row, surface_column)
        surface_cell.value = round(float(surface), 4)
        surface_cell.number_format = "0.0000"
    for column in range(template_columns + 1, 24):
...
def validate_result_workbook(
    ...
    if workbook.sheetnames != expected_names:
        raise ValueError(f"result4.xlsx工作表错误：{workbook.sheetnames}")
    worksheet = workbook.worksheets[0]
    expected_headers: list[float | str] = [
        *np.round(FIXED_OUTPUT_RADII_M * 100.0, 10).tolist(),
        "药材表面",
    ]
    actual_headers = [worksheet.cell(1, column).value for column in range(2, 24)]
    if actual_headers != expected_headers:
        raise ValueError("result4.xlsx固定位置或表面表头不一致")
    expected_last = (
        int(math.floor(solution.end_time_s / OUTPUT_INTERVAL_S) * OUTPUT_INTERVAL_S)
        if solution is not None
        else worksheet.cell(worksheet.max_row, 1).value
    )
    expected_rows = expected_last // 60 + 1
    if worksheet.max_row != expected_rows or worksheet.max_column != 23:
        raise ValueError(
            f"result4.xlsx应为{expected_rows}行×23列，实际为"
    ...
    for excel_row in range(2, worksheet.max_row + 1):
        time_value = (excel_row - 1) * 60
        if worksheet.cell(excel_row, 1).value != time_value:
            raise ValueError(f"result4.xlsx第{excel_row}行时间错误")
        radius_m = None
        source_index = None
        if solution is not None:
            source_index = int(np.searchsorted(solution.time_s, float(time_value)))
            if (
                source_index >= solution.time_s.size
                or abs(solution.time_s[source_index] - time_value) > 1.0e-7
            ):
                raise ValueError(f"正式解中缺少{time_value} s规则输出")
            radius_m = float(solution.radius_m[source_index])
        for fixed_index, fixed_radius in enumerate(FIXED_OUTPUT_RADII_M):
            cell = worksheet.cell(excel_row, fixed_index + 2)
            if cell.alignment.horizontal != "center" or cell.alignment.vertical != "center":
                raise ValueError(f"result4.xlsx单元格{cell.coordinate}未居中")
            if cell.number_format != "0.0000":
                raise ValueError(f"result4.xlsx单元格{cell.coordinate}数值格式错误")
            if solution is not None:
                inside = fixed_radius <= radius_m + 1.0e-12
                expected = solution.moisture_fixed[source_index, fixed_index]
                if inside:
                    inside_count += 1
                    if cell.value is None or not np.isfinite(expected):
                        raise ValueError(f"result4.xlsx域内单元格{cell.coordinate}为空")
                    if float(cell.value) != round(float(expected), 4):
                        raise ValueError(f"result4.xlsx单元格{cell.coordinate}数值不一致")
                    if abs(fixed_radius - radius_m) <= 1.0e-12:
                        equality_count += 1
                        surface_value = worksheet.cell(excel_row, 23).value
                        if float(cell.value) != float(surface_value):
                            raise ValueError("固定位置位于真实表面时两列数值不一致")
                else:
                    blank_count += 1
                    if cell.value is not None:
                        raise ValueError(f"result4.xlsx域外单元格{cell.coordinate}必须为空")
        surface_cell = worksheet.cell(excel_row, 23)
        if surface_cell.value is None:
            raise ValueError(f"result4.xlsx表面单元格{surface_cell.coordinate}为空")
        if surface_cell.number_format != "0.0000":
            raise ValueError(f"result4.xlsx表面单元格{surface_cell.coordinate}数值格式错误")
        if (
            surface_cell.alignment.horizontal != "center"
            or surface_cell.alignment.vertical != "center"
        ):
            raise ValueError(f"result4.xlsx表面单元格{surface_cell.coordinate}未居中")
        time_cell = worksheet.cell(excel_row, 1)
        if time_cell.alignment.horizontal != "center" or time_cell.alignment.vertical != "center":
            raise ValueError(f"result4.xlsx时间单元格{time_cell.coordinate}未居中")
    for column in range(1, 24):
        cell = worksheet.cell(1, column)
        if cell.alignment.horizontal != "center" or cell.alignment.vertical != "center":
            raise ValueError(f"result4.xlsx表头{cell.coordinate}未居中")
    checks = {
        "sheet_names": workbook.sheetnames,
        "rows": worksheet.max_row,
        "columns": worksheet.max_column,
        "first_time_s": 60,
        "last_time_s": expected_last,
        "fixed_radius_cm": [0.0, 2.0, 0.1],
        "surface_column": True,
        "blank_fixed_position_cells": blank_count,
        "inside_fixed_position_cells": inside_count,
        "surface_coincidence_count": equality_count,
    }
    workbook.close()
    return checks
...
def save_run_summary(solution: Q4Solution, data_path: Path, root_audit: dict[str, object]) -> Path:
    ...
    summary = {
        ...
        "formal_output": {"time_interval_s": 60, "last_regular_time_s": int(math.floor(solution.end_time_s / 60.0) * 60), "fixed_radius_cm": [0.0, 2.0, 0.1], "surface_column": True, "xlsx_decimals": 4},
...
def main() -> None:
    ...
    parser.add_argument("--xlsx", type=Path, default=TABLES_DIR / "result4.xlsx")
    ...
    if not roots["all_single_root"]:
        raise RuntimeError("正式根审计发现多个离散根，计算已停止；请检查q4_surface_root_audit.json")
    if not args.no_save:
