# 文件名：q3_validation.py
# 用途：该代码负责第三问模型的网格、时间步长、积分平衡及边界条件敏感性检验。
...
OUTPUT = q3.RESULTS_DIR / "q3_validation.json"
FORMAL_DATA = q3.RESULTS_DIR / "q3_solution.npz"
FORMAL_SUMMARY = q3.RESULTS_DIR / "q3_run_summary.json"
...
def surface_equation_checks(data) -> dict[str, object]:
    grid = q3.q2.make_grid(data["cell_centers_m"].size)
    ambient = q3.load_q3_ambient()
    records = []
    maximum_crossings = 0
...
def main() -> None:
    ...
    if not FORMAL_DATA.exists() or not FORMAL_SUMMARY.exists():
        raise FileNotFoundError("请先运行 q3_main.py 生成所需正式结果文件。")
    ...
    with np.load(FORMAL_DATA) as data:
        ...
        roots = surface_equation_checks(data)
    sensitivity = {
        "role": "辅助敏感性，不参与overall_pass",
        "post_14400_Ca_kg_kg": 0.07,
        "end_time_s": case_results["post_Ca_0.07"]["end_time_s"],
        "end_time_h": case_results["post_Ca_0.07"]["end_time_h"],
    }
    acceptance = {
        ...
        "terminal_maximum_at_axis": abs(
            physical["terminal_high_resolution_max_radius_m"]
        ) <= 1.0e-12,
        "grid_end_time_change_below_60s": convergence["N1025_to_N2561"]["end_time_difference_s"] < 60.0,
        "grid_table_change_below_5e-5": convergence["N1025_to_N2561"]["max_abs_table5_regular_times"] < 5.0e-5,
        ...
        "cumulative_temperature_final_below_1e-4": cumulative["temperature_rate_integral"]["final_relative_residual"] < 1.0e-4,
        "cumulative_moisture_final_below_1e-3": cumulative["moisture_model_integral"]["final_relative_residual"] < 1.0e-3,
    }
    report = {
        ...
        "surface_equation_diagnostic": roots,
    ...
    summary = json.loads(FORMAL_SUMMARY.read_text(encoding="utf-8"))
    summary["validation_file"] = q3.record_path(OUTPUT)
