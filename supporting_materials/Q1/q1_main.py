# 文件名：q1_main.py
# 用途：该代码负责第一问的模型求解、数值结果计算及结果表格输出。
...
DEFAULT_XLSX = ROOT / "result1.xlsx"


def _find_problem_data_root() -> Path:
    """Locate the official problem files from portable relative directories."""
    ...
    raise FileNotFoundError(
        "Cannot locate data/raw/problems or CUMCM2026Problems above q1_main.py"
    )
...
OFFICIAL_TEMPLATE = _find_unique_input("result1.xlsx")
...
def _axis_value(cell_values: np.ndarray, centers_m: np.ndarray) -> np.ndarray:
    """Quadratic-in-r symmetry reconstruction at r=0."""
...
def save_solution_data(solution: Q1Solution) -> Path:
    ...
    path = VALIDATION_DIR / f"q1_solution_n{solution.grid.cells}.npz"
    np.savez_compressed(
        path,
...
def build_result_workbook(
    ...
    """Copy the official template and fill the 1--1800 s output values."""
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    if output_path.resolve() == OFFICIAL_TEMPLATE.resolve():
        raise ValueError("Refusing to overwrite the official result1.xlsx template")
    shutil.copy2(OFFICIAL_TEMPLATE, output_path)
    workbook = load_workbook(output_path)
    if len(workbook.worksheets) != 2:
        raise ValueError("Official result1.xlsx must contain exactly two sheets")
    template_rows = [worksheet.max_row for worksheet in workbook.worksheets]
    template_columns = [worksheet.max_column for worksheet in workbook.worksheets]
    headers = np.round(OUTPUT_RADII_M * 100.0, 10)
    values_by_sheet = (
        np.round(solution.temperature_output_c[1:], 4),
        np.round(solution.moisture_output[1:], 4),
    )
    for sheet_index, (worksheet, values) in enumerate(zip(workbook.worksheets, values_by_sheet)):
        for column, value in enumerate(headers, start=2):
            _extend_template_style(
                worksheet, 1, column, template_rows[sheet_index], template_columns[sheet_index]
            )
            worksheet.cell(1, column, float(value))
        for row, (time_value, profile) in enumerate(zip(solution.time_s[1:], values), start=2):
            if row > template_rows[sheet_index]:
                worksheet.row_dimensions[row].height = worksheet.row_dimensions[
                    template_rows[sheet_index]
                ].height
            _extend_template_style(
                worksheet, row, 1, template_rows[sheet_index], template_columns[sheet_index]
            )
            worksheet.cell(row, 1, int(time_value))
            for column, value in enumerate(profile, start=2):
                _extend_template_style(
                    worksheet, row, column, template_rows[sheet_index], template_columns[sheet_index]
                )
                cell = worksheet.cell(row, column, float(value))
                cell.number_format = "0.0000"
        for column in range(template_columns[sheet_index] + 1, 23):
            letter = worksheet.cell(1, column).column_letter
            source_letter = worksheet.cell(1, template_columns[sheet_index]).column_letter
...
def validate_result_workbook(
    ...
    if workbook.sheetnames != expected_names:
        raise ValueError(f"Unexpected result1.xlsx sheets: {workbook.sheetnames}")
    expected_headers = np.round(OUTPUT_RADII_M * 100.0, 10)
    checks: dict[str, object] = {"sheet_names": workbook.sheetnames}
    for sheet_index, worksheet in enumerate(workbook.worksheets):
        if worksheet.max_row != 1801 or worksheet.max_column != 22:
            raise ValueError(
                f"{worksheet.title}: expected 1801 x 22, got "
                f"{worksheet.max_row} x {worksheet.max_column}"
            )
        headers = np.asarray(
            [worksheet.cell(1, column).value for column in range(2, 23)], dtype=float
        )
        ...
        if solution is not None:
            expected_values = np.round(
                solution.temperature_output_c[1:]
                if sheet_index == 0
                else solution.moisture_output[1:],
                4,
            )
        for excel_row in range(2, 1802):
            if worksheet.cell(excel_row, 1).value != excel_row - 1:
                raise ValueError(f"{worksheet.title}: time mismatch at row {excel_row}")
            for column in range(1, 23):
                ...
                if cell.alignment.horizontal != "center" or cell.alignment.vertical != "center":
                    raise ValueError(f"{worksheet.title}: alignment mismatch at {cell.coordinate}")
                if column >= 2 and cell.number_format != "0.0000":
                    raise ValueError(f"{worksheet.title}: number format mismatch at {cell.coordinate}")
            if expected_values is not None and excel_row in (2, 901, 1801):
                actual = np.asarray(
                    [worksheet.cell(excel_row, column).value for column in range(2, 23)],
                    dtype=float,
                )
                if not np.array_equal(actual, expected_values[excel_row - 2]):
                    raise ValueError(f"{worksheet.title}: value mismatch at row {excel_row}")
...
def save_run_summary(solution: Q1Solution, workbook_path: Path, data_path: Path, figures: list[Path]) -> Path:
    ...
    summary = {
        ...
        "workbook": workbook_path.relative_to(ROOT).as_posix(),
        "solution_data": data_path.relative_to(ROOT).as_posix(),
        "figures": [path.relative_to(ROOT).as_posix() for path in figures],
    }
    VALIDATION_DIR.mkdir(parents=True, exist_ok=True)
    path = VALIDATION_DIR / "q1_run_summary.json"
...
def main() -> None:
    ...
    if not args.no_xlsx:
        build_result_workbook(solution, workbook_path)
