# 文件名：q2_main.py
# 用途：该代码负责第二问变物性传热传质模型的求解、数值结果计算及结果表格输出。
...
FIGURE_DIR = Q2_ROOT / "figures"
VALIDATION_DIR = RESULTS_DIR
DEFAULT_XLSX = Q2_ROOT / "result2.xlsx"


def _find_problem_data_root() -> Path:
    """Locate official problem data from portable relative directories."""
...
def build_result_workbook(
    ...
    """Copy the official template and fill the 1--10800 s output values."""
    ...
    for sheet_index, (worksheet, values) in enumerate((
        (temperature_sheet, temperatures),
        (moisture_sheet, moistures),
    )):
        for column, value in enumerate(headers, start=2):
            source = worksheet.cell(1, min(column, template_columns[sheet_index]))
        ...
        for row, time_value in enumerate(solution.time_s[1:], start=2):
            ...
            source = worksheet.cell(min(row, template_rows[sheet_index]), 1)
            ...
            for column, value in enumerate(values[row - 2], start=2):
                source = worksheet.cell(
                    min(row, template_rows[sheet_index]),
                    min(column, template_columns[sheet_index]),
                )
        ...
        for excel_row, row_values in enumerate(rows, start=2):
            ...
            for column in range(1, 23):
                ...
                if cell.alignment.horizontal != "center" or cell.alignment.vertical != "center":
                    raise ValueError(
                        f"{worksheet.title}: alignment mismatch at {cell.coordinate}"
                    )
                if column >= 2 and cell.number_format != "0.0000":
                    raise ValueError(
                        f"{worksheet.title}: number format mismatch at {cell.coordinate}"
                    )
...
def save_run_summary(
    ...
    summary = {
        ...
        "method": "BDF",
        "input_file": os.path.relpath(ATTACHMENT_1, Q2_ROOT),
        "official_template": os.path.relpath(OFFICIAL_TEMPLATE, Q2_ROOT),
        ...
        "solution_data": data_path.relative_to(Q2_ROOT).as_posix(),
        "result_workbook": DEFAULT_XLSX.relative_to(Q2_ROOT).as_posix(),
